...
const argv = require("yargs")
  .option("server", {
    describe:
      "server to connect to; must be defined in .screeps.yaml servers section",
  })
  .demandOption("server")
  .option("dryrun", {
...
const package_name_underscore = process.env.npm_package_name.replace(
  /\-/g,
  "_"
);
...
function load_config() {
  const yaml_conf = yaml.parse(
    fs.readFileSync(".screeps.yaml", { encoding: "utf8" })
  );
  const configs = yaml_conf.configs || {};

  if (!yaml_conf.servers[argv.server] && argv.server !== "local") {
    console.log(
      `no configuration found for server ${argv.server} in .screeps.yaml`
    );
    return;
  }
  ...
  const copypath = configs["copypath"];

  return { branch, use_terser, extra_options, copypath };
}
...
function load_built_code() {
  ...
  fs.readdirSync("dist").map((filename) => {
    if (filename.endsWith(".wasm")) {
      const data = fs.readFileSync(path.join("dist", filename), {
        encoding: "base64",
      });
...
async function upload(code, server, branch, dryrun) {
  const usage_string = `${code.used_mib.toFixed(
    2
  )} MiB of 5.0 MiB code size limit (${code.used_percent.toFixed(2)}%)`;
  if (dryrun) {
...
async function run() {
  ...
  if (argv.server === "local") {
    if (!config.copypath) {
      console.log("No copy-path specified for local server; skipping");
      return;
    }
    // 本地直接复制 dist 下面的所有文件到本地私服代码目录下
    await fsExtra.copy("dist", config.copypath);
  } else {
